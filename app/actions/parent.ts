...
export async function saveChildProfile(formData: FormData) {
  ...
  const db = getDb();
  const child = db.prepare("SELECT * FROM children WHERE id = ? AND parent_id = ?").get(id, parent.id) as
    | ChildRow
    | undefined;
...
export async function saveFeedback(formData: FormData) {
  ...
  const db = getDb();
  const child = db.prepare("SELECT id FROM children WHERE id = ? AND parent_id = ?").get(childId, parent.id);
  if (!child) redirect("/dashboard");
...
export async function mockCheckout(formData: FormData) {
  ...
  const db = getDb();
  if (seatsRemaining(db) <= 0) redirect("/waitlist");
